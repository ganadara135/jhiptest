...
describe('JobHistory Screen Tests', () => {
  ...
  it('should allow you to create, update, and delete the JobHistory entity', async () => {
    ...
    await setDateTimePickerValue('startDateInput', '2021-01-25T16:12:00+09:00', 'ISO8601');
    await scrollTo('endDateInput', 'jobHistoryEditScrollView');
    await setDateTimePickerValue('endDateInput', '2021-01-24T21:21:00+09:00', 'ISO8601');
    ...
    jestExpect(Date.parse(startDateCreateAttributes.label)).toEqual(Date.parse('2021-01-25T16:12:00+09:00'));
    await scrollTo('endDate', 'jobHistoryDetailScrollView');
    const endDateCreateAttributes = await element(by.id('endDate')).getAttributes();
    jestExpect(Date.parse(endDateCreateAttributes.label)).toEqual(Date.parse('2021-01-24T21:21:00+09:00'));
    ...
    await waitForElementToBeVisibleById('jobHistoryEditScrollView');
    await scrollTo('startDateInput', 'jobHistoryEditScrollView');
    await setDateTimePickerValue('startDateInput', '2021-01-25T14:59:00+09:00', 'ISO8601');
    await scrollTo('endDateInput', 'jobHistoryEditScrollView');
    await setDateTimePickerValue('endDateInput', '2021-01-25T05:48:00+09:00', 'ISO8601');
    await scrollTo('languageInput', 'jobHistoryEditScrollView');
    ...
    jestExpect(Date.parse(startDateUpdateAttributes.label)).toEqual(Date.parse('2021-01-25T14:59:00+09:00'));
    await scrollTo('endDate', 'jobHistoryDetailScrollView');
    const endDateUpdateAttributes = await element(by.id('endDate')).getAttributes();
    jestExpect(Date.parse(endDateUpdateAttributes.label)).toEqual(Date.parse('2021-01-25T05:48:00+09:00'));
    await scrollTo('language', 'jobHistoryDetailScrollView');
